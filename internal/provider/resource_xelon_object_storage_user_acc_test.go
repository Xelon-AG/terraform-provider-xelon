...
func TestAccResourceXelonObjectStorageUser(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			// Import the user and verify all state can be reconstructed.
			{
				ResourceName: "xelon_object_storage_user.test",
				ImportState:  true,
				ImportStateIdFunc: func(s *terraform.State) (string, error) {
					rs, ok := s.RootModule().Resources["xelon_object_storage_user.test"]
					if !ok {
						return "", fmt.Errorf("not found: xelon_object_storage_user.test")
					}
					region := rs.Primary.Attributes["region"]
					id := rs.Primary.Attributes["id"]
					return fmt.Sprintf("%v/%v", region, id), nil
				},
